...
namespace BinaryTrees
{
    ...
    public class BinaryTree<TKey, TValue> where TKey : IComparable<TKey>
    {
        ...
        private BinaryTreeNode<TKey, TValue> AddBalanced(TKey[] keys, TValue[] values, int start, int end)
        {
            ...
            BinaryTreeNode<TKey, TValue> nodo = new BinaryTreeNode<TKey, TValue>(keys[center], values[center]);
            nodo.LeftChild = AddBalanced(keys, values, start, center - 1);
            nodo.RightChild = AddBalanced(keys, values, center + 1, end);
            return nodo;
        }
